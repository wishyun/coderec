...
<?php					$image 		= $one['image'];  ?>
			<?php   	if($image):  ?>
				<li id="gal1">
					<a data-toggle="modal"  onmouseover="$(this).tooltip('show')" data-placement="top" data-original-title="<?=Yii::$service->page->translate->__($val)?>" data-image="<?= Yii::$service->product->image->getResize($image,$middle_img_width,false) ?>"  data-zoom-image="<?= Yii::$service->product->image->getUrl($image);  ?>"  attr="<?= $attr ?>"  class="imgshow active_v"  value="<?= $key ?>">
						<img src="<?= Yii::$service->product->image->getResize($image,[50,50],false) ?>" /></a>
					<b></b>
				</li>
			<?php   	else: ?>
				<li>
